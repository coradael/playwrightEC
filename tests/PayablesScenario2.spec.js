...
import { logExcel } from "../logs/logToExcel.ts"; // Import the logging function
...
test(testName, async ({ page }) => {
  ...
  // await page.waitForSelector("text=Sign in"); // without login.js
  // await page.waitForSelector("text=Stay signed in?"); // without login.js
  ...
  await logStep(sheet, "Projec velden zijn ingevuld", async () => {
    ...
    await page.getByTitle("Search: Task Number").click();
    ...
    await page.getByTitle("Search: Task Number").click();
    await page.getByTitle("Search: Expenditure Type").click();
    await page.getByText("Inhuur externen", { exact: true }).first().click();
    await page.getByTitle("Search: Expenditure Organization").click();
    await page.getByText("Team Externe Inhuur Doorleen").first().click();
  });
  await logStep(sheet, "Validatie is uitgevoerd", async () => {
    ...
    logExcel("Validate is uitgevoerd");
  });
});
test.afterAll(async () => {
  await saveWorkbook();
