...
test(testName, async ({ page }) => {
  ...
  //logExcel("Naar 'Invoices' geklikt");
  await page.waitForTimeout(3000);
  await page.getByRole("link", { name: "Tasks" }).click();
  await page.getByRole("button", { name: "Create" }).click();
  //logExcel("Create invoice gestart");
  await page.getByTitle("Search: Business Unit").click();
  await page.getByText("Profource BV").first().click();
  //logExcel("Business Unit is geselecteerd: Profource BV");
  ...
  //logExcel("Supplier Albert is gevonden");
  await page
    .getByRole("textbox", { name: "Supplier", exact: true })
    .press("Enter");
  ...
  await page.getByRole("button", { name: "Expand Lines" }).click();
  //logExcel("Expand Lines zijn ingevuld");
  await page
